...
nf_check() {
    local result
    result=$(curl -${1}fsL --user-agent "${UA_Browser}" --write-out %{http_code} --output /dev/null --max-time 10 "https://www.netflix.com/title/70143836" 2>&1)
    if [[ "$result" == "404" ]]; then 
        echo "遗憾，当前IP仅解锁Netflix自制剧"
    elif [[ "$result" == "403" ]]; then
        echo "杯具，当前IP不能看Netflix"
    elif [[ "$result" == "200" ]]; then
        echo "恭喜，当前IP完整解锁Netflix非自制剧"
    else
        echo "死心吧，Netflix不服务当前IP地区"
    fi
}
